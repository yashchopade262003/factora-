...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ResponseStructure<String>> handleDispatchException(DispatchException ex) {
	...
	public ResponseEntity<ResponseStructure<String>> handleInventoryServiceException(InventoryServiceException ex) {
		return buildResponse(ex.getStatus(), ex.getMessage());
	}
	...
	// Most common cause in practice: an optional numeric/date field (e.g.
	// buyerOrderId, finishedGoodsInventoryId, dispatchDate) was sent as an
	// empty string "" instead of being omitted/null. Jackson can't coerce
	// "" into a Long/LocalDate and throws here. The frontend now sanitizes
	// outgoing payloads to avoid this, but this message stays specific so
	// any other client hitting the same issue gets an actionable error
	// instead of a bare "Malformed request body".
